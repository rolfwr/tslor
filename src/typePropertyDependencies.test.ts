...
`;
  ...
  const actualTargetSource = generateNewModuleSource(
    symbolDefinitions,
    requiredImports,
  );
  ...
  let actualSourceSource = removeSymbolsFromSource(
    sourceFile,
    allSymbolsToMove,
  );
  ...
  actualSourceSource = removeUnusedImports(
  ...
  actualSourceSource = addImportForMovedSymbols(
  ...
  assert.include(
    actualTargetSource,
    'export interface DerivedClip',
    'Target should have DerivedClip',
  );
  assert.include(
    actualTargetSource,
    'export interface VirtualClip',
    'Target should have VirtualClip',
  );
  assert.include(
    actualTargetSource,
    'export interface Item',
  ...
  assert.include(
    actualTargetSource,
    'virtualClipInfo?: VirtualClip',
    'Item should reference VirtualClip',
  );
  ...
  assert.include(
    actualSourceSource,
    'export interface OtherType',
    'Source should keep OtherType',
  );
  assert.include(
    actualSourceSource,
    'from "./itemType"',
    'Source should import from itemType',
  );
  assert.include(
    actualSourceSource,
    'export {',
    'Source should re-export moved symbols',
  );
  assert.include(
    actualSourceSource,
    'VirtualClip',
    'Source should re-export VirtualClip',
  );
  assert.include(
    actualSourceSource,
    'DerivedClip',
    'Source should re-export DerivedClip',
  );
  ...
  assert.notInclude(
    ...
    'Source should not have VirtualClip definition',
  );
  assert.notInclude(
    actualSourceSource,
    'interface Item {',
    'Source should not have Item definition',
  );
...
`;
  ...
  const targetSource = generateNewModuleSource(
    symbolDefinitions,
    requiredImports,
  );
  ...
  const itemFileAfterRemoval = project.createSourceFile(
    `temp-after-removal-${Date.now()}.ts`,
    modifiedItemSource,
  );
  modifiedItemSource = removeUnusedImports(
    itemFileAfterRemoval,
    onlyUsedByTarget,
  );

  const itemFileAfterCleanup = project.createSourceFile(
    `temp-after-cleanup-${Date.now()}.ts`,
    modifiedItemSource,
  );
  modifiedItemSource = addImportForMovedSymbols(
    itemFileAfterCleanup,
    allSymbolsToMove,
    './itemType',
    true,
  );
...
`;
  ...
  console.log(
    'Moving ComplexItem requires:',
    Array.from(allSymbolsToMove).sort(),
  );

  assert.isTrue(allSymbolsToMove.has('ComplexItem'), 'Should move ComplexItem');
  assert.isTrue(
    allSymbolsToMove.has('MiddleType'),
    'Should move MiddleType (referenced by ComplexItem.info)',
  );
  assert.isTrue(
    allSymbolsToMove.has('BaseType'),
    'Should move BaseType (extended by MiddleType)',
  );
  assert.isTrue(
    allSymbolsToMove.has('NestedData'),
    'Should move NestedData (referenced by MiddleType.data)',
  );
  assert.isTrue(
    allSymbolsToMove.has('Metadata'),
    'Should move Metadata (referenced by NestedData.metadata)',
  );

  assert.isFalse(
    allSymbolsToMove.has('UnrelatedType'),
    'Should NOT move UnrelatedType',
  );
...
`;
  ...
  const sourceAfterRemoval = project.createSourceFile(
    `temp-after-removal-${Date.now()}.ts`,
    actualSource,
  );
  actualSource = removeUnusedImports(sourceAfterRemoval, onlyUsedByTarget);
  const sourceAfterCleanup = project.createSourceFile(
    `temp-after-cleanup-${Date.now()}.ts`,
    actualSource,
  );
  ...
  if (finalDiagnostics.length > 0) {
    ...
    for (const diag of finalDiagnostics) {
      ...
      console.error(
        `  ${file?.getFilePath()}:${lineNumber?.line}: ${diagMessageText(diag)}`,
      );
    }
  }
