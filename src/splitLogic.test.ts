...
function performSplitInMemory(
  ...
  for (const symbol of symbolsToMove) {
    ...
    if (!analysis.canSplit) {
      errors.push(
        `Cannot split '${symbol}': circular dependencies with ${analysis.circularDependencies.join(', ')}`,
      );
      continue;
    }
  ...
  const exportedSymbolsToMove = Array.from(allSymbolsToMove).filter((s) =>
    dependencies.exports.has(s),
  );
  if (
    exportedSymbolsToMove.length === dependencies.exports.size &&
    dependencies.exports.size > 0
  ) {
  ...
  const cleanedSource = removeUnusedImports(tempSource, onlyUsedByMovedSymbols);

  // Add imports for re-exports
  const symbolsToReExport = new Set(
    Array.from(allSymbolsToMove).filter((s) => dependencies.exports.has(s)),
  );

  if (symbolsToReExport.size > 0) {
    const tempSource2 = project.createSourceFile(
      '__temp_source2.ts',
      cleanedSource,
      { overwrite: true },
    );
...
`,
  ...
  const result = performSplitInMemory(project, sourceFile, '/test/target.ts', [
    'nonExistentFunction',
  ]);
...
`,
  ...
  assert.notMatch(
    ...
    /^import.*hello/m,
    'Should not have import when only re-exporting',
  );
...
`,
  ...
  assert.notMatch(
    result.sourceContent,
    /^import.*formatUser/m,
    'Should not import re-exported symbol',
  );
